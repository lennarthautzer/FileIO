...
bool SyntaxHandler::startsWith( std::string const & in,
    std::string const & prefix ) const
{
    return in.substr( 0, prefix.length() ) == prefix ? true : false;
}

bool SyntaxHandler::startsWith( char const * const & in,
    char const * const & prefix ) const
{
    return startsWith( std::string( in ), std::string( prefix ) );
}

bool SyntaxHandler::endsWith( std::string const & in,
...
bool SyntaxHandler::endsWith( char const * const & in,
    char const * const & suffix ) const
{
    return endsWith( std::string( in ), std::string( suffix ) );
}

std::string SyntaxHandler::centerString( std::string const & str,
    int const targetSize ) const
{
    ...
    return whiteSpace > 0 ? ( std::string( whiteSpace / 2, ' ' ) + str +
        std::string( whiteSpace / 2, ' ' ) ) : str;
}

std::vector<std::string> SyntaxHandler::splitString( std::string str,
...
SyntaxHandler::splitStringsInQuotes( std::string str ) const
{
    size_t i = 0;
    std::vector<std::string> ret;
...
void SyntaxHandler::stripChar( std::string & str,
    char const & charToRemove ) const
{
